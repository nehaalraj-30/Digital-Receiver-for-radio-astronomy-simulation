...
digital_values_no_offset = np.array((digital_values_no_offset)*2)/2**(n_bits)

w_signal,gain = window_bits(digital_values_no_offset,n_bits)
windowed = digital_values_no_offset * w_signal
...
plt.step(adc_time*1e6, digital_values_no_offset, where ='post')
plt.xlabel('time(us)')
...
# plt.step(adc_time*1e6,w_signal,where = 'post')
# plt.xlabel('t(us)')
# plt.ylabel('amplitude')
